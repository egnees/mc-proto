...
use crate::{mc, model, Address, Process};
...
pub fn make_build(
    ...
    ping: impl Fn() -> Rc<RefCell<dyn Process>> + Clone + Sync + Send + 'static,
    pong: impl Fn() -> Rc<RefCell<dyn Process>> + Clone + Sync + Send + 'static,
    locals: usize,
) -> impl mc::ApplyFn {
    move |sys| {
        // configure network
