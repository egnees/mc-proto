...
pub fn check_someone_deliver(s: model::SystemHandle, nodes: usize) -> Result<usize, String> {
    for node in 0..nodes {
...
pub fn check_casual_order(s: model::SystemHandle, nodes: usize) -> Result<(), String> {
    let mut checker = CausalChecker::new(nodes);
    for log in s.log().iter() {
        match log {
            model::LogEntry::ProcessSentLocalMessage(msg) => {
                let node: usize = msg.process.node.parse().unwrap();
                if msg.content != "connect" {
                    checker.deliver(node, &msg.content)?;
                }
            }
            model::LogEntry::ProcessReceivedLocalMessage(msg) => {
                let node: usize = msg.process.node.parse().unwrap();
                if msg.content != "connect" {
...
pub fn check_uniform_agreement(s: model::SystemHandle, nodes: usize) -> Result<(), String> {
    let mut messages = HashSet::new();
    for log in s.log().iter() {
        if let model::LogEntry::ProcessSentLocalMessage(msg) = log {
            if msg.content != "connect" {
...
pub fn check_validity(s: model::SystemHandle) -> Result<(), String> {
    let mut messages: HashMap<String, HashSet<String>> = Default::default();
    for log in s.log().iter() {
        match log {
            model::LogEntry::NodeCrashed(e) => {
                messages.remove(&e.node);
            }
            model::LogEntry::ProcessReceivedLocalMessage(e) => {
            ...
            model::LogEntry::ProcessSentLocalMessage(e) => {
                messages
                    .entry(e.process.node.clone())
                    .or_default()
                    .remove(&e.content);
...
pub fn check_validity_and_agreement(s: model::SystemHandle, nodes: usize) -> Result<(), String> {
