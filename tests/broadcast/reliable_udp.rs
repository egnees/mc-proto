...
impl Bcast {
    ...
    fn others<'a>(&'a self) -> impl Iterator<Item = mc::Address> + 'a {
        (0..self.proc_cnt)
            .filter(|x| *x != self.me)
...
impl mc::Process for Bcast {
    fn on_message(&mut self, from: mc::Address, orign_content: String) {
            ...
            Message::Bcast(content) => {
                ...
                if insert_result {
                    ...
                    for other in self.others() {
                        if other != from {
                            mc::send_message(&other, orign_content.clone());
                        }
                    }
                }
    ...
    fn on_local_message(&mut self, content: String) {
        ...
        let msg = serde_json::to_string(&msg).unwrap();
        for other in self.others() {
            mc::send_message(&other, msg.clone());
        }
    }
